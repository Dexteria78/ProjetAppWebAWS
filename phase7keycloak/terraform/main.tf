    ...
    widgets = [
      {
        ...
        properties = {
          metrics = [
            # Ajout de la dimension "LoadBalancer" et de son suffixe ARN requis par CloudWatch
            ["AWS/ApplicationELB", "TargetResponseTime", "LoadBalancer", aws_lb.application.arn_suffix, { stat = "Average" }],
      ...
      },
      {
        ...
        properties = {
          metrics = [
            # Syntaxe CloudWatch JSON correcte pour la dimension de l'ASG
            ["AWS/EC2", "CPUUtilization", "AutoScalingGroupName", aws_autoscaling_group.web_servers.name, { stat = "Average" }]
          ]
          period = 300
          stat   = "Average"
          region = var.aws_region
...
# 1. Groupe de sécurité dédié pour Keycloak
resource "aws_security_group" "keycloak" {
  name        = "student-records-keycloak-sg-phase6"
  description = "Security group for Keycloak instance"
  vpc_id      = data.aws_vpc.default.id

  # Autorise le trafic sur le port 8080 (Keycloak) uniquement depuis l'ALB
  ingress {
  ...
  # Autorise SSH (facultatif, pour le debug)
  ingress {
    description = "SSH from anywhere"
    from_port   = 22
    to_port     = 22
...
#2. L'instance EC2 pour Keycloak
resource "aws_instance" "keycloak1" {
  ami                         = data.aws_ami.amazon_linux_2023.id
  instance_type               = "t3.small"
  subnet_id                   = data.aws_subnets.default.ids[0]
  
  # 1. On force la création d'une IP publique (INDISPENSABLE pour SSH)
  associate_public_ip_address = true 
  
  # 2. On attache la clé SSH
  # Si vous êtes sur AWS Academy, laissez "vockey". 
  # Sinon, mettez le nom exact de votre clé AWS (sans le .pem).
  key_name                    = "vockey" 
...
root_block_device {
    volume_size = 20    # 20 Go au lieu de 8 Go
    volume_type = "gp3" # Type de disque moderne et performant
}
  ...
  tags = {
    Name    = "student-records-keycloak-phase6"
    Phase   = "6"
    Project = "StudentRecords"
  }
}

# (Optionnel) Affiche l'IP publique à la fin du déploiement pour vous faciliter la vie
output "keycloak_public_ip" {
  description = "Adresse IP publique pour se connecter en SSH à Keycloak"
  value       = aws_instance.keycloak1.public_ip
}

# 3. Le Target Group pour Keycloak (Port 8080)
resource "aws_lb_target_group" "keycloak" {
  name     = "student-records-tg-kc-phase6"
  port     = 8080
  protocol = "HTTP"
  vpc_id   = data.aws_vpc.default.id
...
health_check {
    enabled             = true
    path                = "/"          # On vérifie simplement la racine du site
    matcher             = "200-399"    # On accepte les succès (200) ET les redirections (302)
    interval            = 60
    timeout             = 10
    healthy_threshold   = 2
  ...
  tags = {
    Name    = "student-records-tg-keycloak-phase6"
    Phase   = "6"
    Project = "StudentRecords"
  }
}

# 4. Attachement de l'instance au Target Group
resource "aws_lb_target_group_attachment" "keycloak" {
  target_group_arn = aws_lb_target_group.keycloak.arn
  target_id        = aws_instance.keycloak1.id
  port             = 8080
}

# 5. La règle de routage sur votre ALB existant
resource "aws_lb_listener_rule" "keycloak" {
  listener_arn = aws_lb_listener.http.arn # Réutilise votre listener HTTP 
  priority     = 50
  ...
  # Intercepte tous les chemins utilisés par Keycloak
  condition {
    path_pattern {
      values = ["/admin/*", "/realms/*", "/resources/*", "/js/*"]
    }
  }
}
